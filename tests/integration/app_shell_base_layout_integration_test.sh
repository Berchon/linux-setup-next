...
app_shell_init_framebuffer 40 12
app_shell_set_message_bar "base status"
app_shell_render_base_layout

assert_eq "$(cell_buffer_get_cell front 6 1)" "+|6|15|0" "header should render panel border with light blue on white"
assert_eq "$(cell_buffer_get_cell front 8 3)" "L|6|15|1" "header should render centered title text"
assert_eq "$(cell_buffer_get_cell front 0 7)" "+|6|0|0" "central area should render panel border below header"
assert_eq "$(cell_buffer_get_cell front 1 6)" ".|7|0|0" "background tile should remain visible around header and content"
