...
assert_contains() {
  ...
  if [[ "${actual}" != *"${expected_substring}"* ]]; then
    printf "FAIL: %s\nexpected substring: %q\nactual:             %q\n" "${message}" "${expected_substring}" "${actual}" >&2
    exit 1
  fi
}
...
assert_contains "${success_toast}" "Saved app.language=en" "language switch toast should use active catalog after runtime update"
...
assert_contains "${error_toast}" "Failed to save app.language" "error toast should use current active catalog"

rm -f "${test_config_file}"
printf "PASS: ui state i18n toast feedback tests\n"
